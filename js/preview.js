...
const Preview = (() => {
  ...
  let contentAspect = null; // representative aspect for the current PDF(s)
  ...
  async function build() {
    ...
    contentAspect = await TrimBox.getRepresentativeAspect(active);
  ...
  async function renderPage(index) {
    ...
    for (let i = 0; i < slots.length; i += 1) {
      ...
      const dataUrl = await Thumbnails.render(slide.docIndex, slide.pageIndex, 640).catch(() => null);
      if (!dataUrl) continue;
