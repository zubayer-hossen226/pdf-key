/* Z Key — thumbnails.js
   ...
   Every page is cropped using its document's single uniform TrimBox, so
   thumbnails from the same source line up consistently. */

const Thumbnails = (() => {
  const cache = new Map(); // key: `${docIndex}:${pageIndex}:${targetWidth}` -> dataURL
  const inFlight = new Map();
  const MAX_CACHE_ENTRIES = 60;

  async function render(docIndex, pageIndex, targetWidth = 220) {
    const key = `${docIndex}:${pageIndex}:${targetWidth}`;
    if (cache.has(key)) {
      const val = cache.get(key);
      cache.delete(key);
      cache.set(key, val); // refresh recency
      return val;
    }
  ...
  function attachLazy(container, docIndex, pageIndex, targetWidth = 220) {
    const skeleton = document.createElement('div');
    skeleton.className = 'page-thumb-skeleton';
    container.appendChild(skeleton);

    const load = () => {
      render(docIndex, pageIndex, targetWidth)
        .then((dataUrl) => {
