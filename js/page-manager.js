...
const PageManager = (() => {
  ...
  function buildSourceCard(slide) {
    ...
    Thumbnails.attachLazy(thumbWrap, slide.docIndex, slide.pageIndex, 260);
    ...
    const state = AppState.get();
    if (state.sourceDocs.length > 1) {
      const fileTag = document.createElement('span');
      fileTag.className = 'page-tag';
      fileTag.style.background = 'rgba(124,92,255,0.18)';
      fileTag.style.color = 'var(--c-purple)';
      fileTag.textContent = `File ${slide.docIndex + 1}`;
      footer.appendChild(fileTag);
    }
    if (slide.removed) {
