/* Z Key — color-mode.js
   ...
   directly (vector/text preserved) via pdf-lib's embedPage.
   ...
   print, crops it to the document's uniform TrimBox, and converts it to a
   clean print-friendly grayscale — inverting dark-themed slides (dark
   background, light text) so the result is always dark text on white
   paper, never a black page. */

const ColorMode = (() => {
  const PRINT_DPI = 260; // sharp text/diagrams for print
  ...
  // Samples the 4 corner patches of a rendered region to estimate its
  // background brightness — used to decide whether a page is a "dark
  ...
  // be a dark-themed slide — inverted, so the printed result is always
  // dark text on white paper, never a black page.
  ...
  async function rasterizeGrayscalePng(docIndex, pageIndex, slotWidthPt, slotHeightPt) {
    const doc = AppState.pdfJsDocFor(docIndex);
    const box = await TrimBox.getCropBox(docIndex);
    ...
    const targetWidthPx = ptToPx(slotWidthPt, PRINT_DPI);
    const targetHeightPx = ptToPx(slotHeightPt, PRINT_DPI);
